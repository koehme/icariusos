...
void test_read(const char file[])
{
	...
	if (vfs_fseek(fd, 0x2300, SEEK_SET) < 0) {
		kprintf("[ERROR] Failed to Seek in File Descriptor: %d\n", fd);
		vfs_fclose(fd);
		return;
	};

	if (vfs_fread(buffer, 10, 1, fd) <= 0) {
		kprintf("[ERROR] Failed to Read from File Descriptor: %d\n", fd);
		vfs_fclose(fd);
		return;
	};
...
void test_readdir(const char* path)
{
	...
	if (fd < 0) {
		kprintf("Error: Could not open %s\n", path);
		return;
	};
	kprintf("\n%s\n", path);
	kprintf("========================\n");
...
void test_write(char msg[], const uint32_t bytes)
{
	...
	vfs_fclose(fd);
	kprintf("[FAT16] LOG.TXT was successfully written.\n");
	return;
};
