...
uint32_t *page_init_directory(const uint8_t flags, const int page_size, const int entries)
{
    ...
    uint32_t *s_address_directory = kcalloc(sizeof(uint32_t) * entries);
    ...
    for (int directory_index = 0; directory_index < entries; directory_index++)
    {
        ...
        s_address_directory[directory_index] = (uint32_t)page_table | flags | PAGE_WRITEABLE;
    };
    return s_address_directory;
};
